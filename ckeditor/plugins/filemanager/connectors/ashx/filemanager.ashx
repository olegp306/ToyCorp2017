...
public class filemanager : AdvantShop.Core.HttpHandlers.AdminHandler, IHttpHandler
{
    ...
    private string getFolderInfo(string path)
    {
        ...
        foreach (FileInfo fileInfo in RootDirInfo.GetFiles())
        {
            ...
            if (IsImage(fileInfo))
            {
                sb.AppendLine("\"Preview\": \"" + Path.Combine(BaseUrl, path, fileInfo.Name) + "\",");
            }
            else
            {
                sb.AppendLine("\"Preview\": \"" +
                                String.Format("{0}{1}.png", IconDirectory, fileInfo.Extension.Replace(".", "")) +
                                "\",");
            }
    ...
    private string getInfo(string path)
    {
        ...
        else
        {
            ...
            if (IsImage(fileInfo))
            {
                sb.AppendLine("\"Preview\": \"" + BaseUrl + path + "\",");
            }
            else
            {
    ...
    public new void ProcessRequest(HttpContext context)
    {
        ...
        try
        {
            ...
            BaseUrl = context.Request.Url.GetLeftPart(UriPartial.Authority) + context.Request.ApplicationPath + (!context.Request.ApplicationPath.EndsWith("/") ? "/" : string.Empty);
        ...
        {
            AdvantShop.Diagnostics.Debug.LogError(err);
        }
    }
}